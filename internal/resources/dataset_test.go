...
// Test Create with only uid/gid (no mode) includes options.stripacl
func TestDatasetResource_Create_WithUIDGIDOnly(t *testing.T) {
	var setpermCalled bool
	var setpermParams map[string]any
	...
	// Only uid/gid specified, no mode
	planValue := createDatasetResourceModelWithPerms(nil, "storage", "apps", nil, nil, nil, "lz4", nil, nil, nil, nil, nil, int64(1000), int64(1000))
	...
	// When no mode is specified, options.stripacl should be set
	options, ok := setpermParams["options"].(map[string]any)
	if !ok {
		t.Fatal("expected options to be set when mode is not specified")
	}

	if options["stripacl"] != false {
		t.Errorf("expected options.stripacl to be false, got %v", options["stripacl"])
	}

	// mode should NOT be in params when not specified
	if _, hasMode := setpermParams["mode"]; hasMode {
		t.Error("mode should not be set when not specified in config")
	}
