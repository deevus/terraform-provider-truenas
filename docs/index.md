...
The TrueNAS provider enables Terraform to manage TrueNAS SCALE and Community editions through the midclt API over SSH or WebSocket.
...
The provider supports two authentication methods: SSH (key-based) and WebSocket (API key).
...
### WebSocket Authentication (Recommended for Large Deployments)

WebSocket transport uses a persistent JSON-RPC connection that authenticates once. This avoids SSH authentication rate limits (TrueNAS limits to 20 attempts per 60 seconds).

**When to use WebSocket:**
- Large Terraform configurations with many resources
- CI/CD pipelines making frequent API calls
- Any deployment hitting SSH rate limits
...
provider "truenas" {
  ...
  websocket {
    username = "root"              # TrueNAS username (usually "root")
    api_key  = var.truenas_api_key
    # Optional settings:
    # port                 = 443       # Default: 443
    # insecure_skip_verify = false     # Default: false
    # max_concurrent       = 20        # Default: 20
    # connect_timeout      = 30        # Default: 30 seconds
    # max_retries          = 3         # Default: 3
  }

  # SSH is still required for file operations (read, delete)
  ssh {
    private_key          = file("~/.ssh/truenas_ed25519")
...
3. Click **Add**
4. Give the key a descriptive name (e.g., "Terraform")
5. Copy the generated key - you won't be able to see it again

Store the API key securely (e.g., in a Terraform variable or secrets manager).
...
- `auth_method` (String) Authentication method. Valid values are 'ssh' or 'websocket'.
...
- `websocket` (Block, Optional) WebSocket connection configuration. (see [below for nested schema](#nestedblock--websocket))
